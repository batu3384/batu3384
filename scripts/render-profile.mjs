...
const USER = "batu3384";
const CALDER_SHOT =
  "https://raw.githubusercontent.com/batu3384/calder/main/docs/images/screenshots/workspace-mosaic.png";
const SNAKE = {
...
<a href="${repoUrl(item.repo)}">
  ...
  const supportPins = [secondary[0], secondary[1], shipping[0]]
    .map((item) => {
      const dark = pinCard(item, "github_dark");
      const light = pinCard(item, "github_light");
      return `<p align="center">
<a href="${repoUrl(item.repo)}">
  <picture>
    <source media="(prefers-color-scheme: dark)" srcset="${dark}">
    <img src="${light}" alt="${item.repo} — ${item.purpose}" width="320">
  </picture>
</a>
</p>`;
    })
...
<p align="center">
  <a href="${repoUrl("calder")}">
    <img src="${CALDER_SHOT}" alt="Calder — parallel Claude Code, Codex, Cursor, and Antigravity sessions in one workspace" width="100%">
  </a>
</p>
...
<p align="center">
  <picture>
    ...
    <img alt="Contribution graph snake animation" src="${SNAKE.light}" width="100%">
  </picture>
</p>
